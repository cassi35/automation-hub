...
def test_start_execution_cria_execution_com_status_process(db_session, db_handler):
    automation = AutomationModel(slug="job-orq", name="job-orq", trigger="system", status="active")
...
def test_fluxo_completo_finish_execution(db_session, db_handler):
    automation = AutomationModel(slug="job-fluxo", name="job-fluxo", trigger="system", status="active")
    db_session.add(automation)
    db_session.commit()
