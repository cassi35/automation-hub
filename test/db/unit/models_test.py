...
# from test.fixtures.fixture_models import automation_model, execution_model, metric_model, step_model
# from test.fixtures.connection_fixture import db_handler,db_session

# @pytest.mark.unit # marcador personalizado para fazer testes rapidos usado para -m
@pytest.mark.integration
def test_cria_arvore_completa(db_session, automation_model, execution_model, step_model, metric_model):
    """aqui testa o relacionamento completo entre as entidades"""
    db_session.add(automation_model)
    db_session.commit()

    saved = db_session.get(AutomationModel, automation_model.id)
    automation_json = {
        "name": saved.name,
...
@pytest.mark.integration
def test_cascade_delete_automation_apaga_executions(db_session, automation_model, execution_model):
    """testa se ao apagar uma automation, também apaga suas executions"""
    db_session.add(automation_model)
    db_session.commit()

    execution_id = execution_model.id
    db_session.delete(automation_model)
    db_session.commit()
...
@pytest.mark.integration
def test_cascade_delete_execution_apaga_steps(db_session, automation_model, execution_model, step_model):
    """testa se ao apagar uma execution, também apaga seus steps"""
    db_session.add(automation_model)
    db_session.commit()

    step_id = step_model.id
    db_session.delete(execution_model)
    db_session.commit()
